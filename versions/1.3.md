...
- new property: [`queryString.encoding`](#querystring)
...
| **comment**   | `string`  | ✖️        | `N/A`   | A comment provided by the user or the application                  |
| **encoding**  | `string`  | ✖️        | `N/A`   | *(new in 1.3)* - Encoding used for request `value` field e.g "base64" |
> - Although `encoding` should usually be `identity` most HAR producers only produce `url` encoded values.
...
| [**headers**](#headers)| `string`  | ✖️        | `N/A`   | *(new in 1.3)* - Headers attached to a `multipart/form-data` request parameter. |
| **comment**       | `string`  | ✖️        | `N/A`   | A comment provided by the user or the application                   |
> * If `request.postData.mimeType` is not `multipart/form-data`, then `headers` must be absent.
> * `headers` should not be confused with the request headers, it is attached to each parameter in the form data.
...
> - Leave out the `encoding` field if the text field is HTTP decoded (decompressed & unchunked), then trans-coded from its original character set into `UTF-8`
> - Before setting the text field, the HTTP response is decoded (decompressed & unchunked), then trans-coded from its original character set into `UTF-8`.
...
> - `encoding` field is useful for including binary responses (e.g. images) into the HAR file.
> - `encoding` field should be thought of as having one of the following values:
>   - `identity` - The default value, meaning no encoding method is used, or it is completely decoded.
>   - `base64` - Specified by [RFC 4648](https://tools.ietf.org/html/rfc4648), used for binary content, used by some HAR producers without specifying encoding. If this encoding method is used, then `encoding` MUST be set to `base64`.
>   - `url` - Specified by [RFC 3986](https://tools.ietf.org/html/rfc3986#section-2.1), and used by some HAR producers as the default encoding, instead of decoding to UTF-8. If this encoding method is used, then `encoding` MUST be set to `url`.
